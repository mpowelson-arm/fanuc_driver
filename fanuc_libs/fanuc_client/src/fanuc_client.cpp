...
void FanucClient::stopRealtimeStream()
{
  ...
  std::cout << "Sending final Stream Motion command with last_data=1 before shutdown." << std::endl;
  stream_motion_->sendCommand(command_pos, true, {});

  // Give the controller one more status cycle to acknowledge the final command.
  stream_motion::RobotStatusPacket status;
  bool got_status_after_last_command = false;
  const auto settle_pre_loop_time = std::chrono::steady_clock::now();
  while (std::chrono::steady_clock::now() - settle_pre_loop_time <= std::chrono::seconds(1))
  {
    if (stream_motion_->getStatusPacket(status))
    {
      got_status_after_last_command = true;
      std::cout << "Status after last_data=1: sequence_no=" << status.sequence_no
                << " status=0x" << std::hex << static_cast<int>(status.status)
                << " robot_status=0x" << static_cast<int>(status.robot_status) << std::dec << std::endl;
      break;
    }
  }
  if (!got_status_after_last_command)
  {
    throw std::runtime_error(kStatusPacketFailureMessage);
  }
